...
import { TrendingUp, TrendingDown, DollarSign, Activity, Zap, Target, Sparkles, ArrowUp, ArrowDown } from 'lucide-react';
      ...
      <div className="text-center max-w-5xl mx-auto relative z-10">
        ...
        >
          ...
          >
            ...
            <span className="text-sm text-orange-300 relative z-10">Live Market Sentiment Analysis</span>
          ...
          >
            <motion.span
              className="bg-gradient-to-r from-white via-orange-200 to-orange-400 bg-clip-text text-transparent inline-block"
              animate={{
            ...
            <br />
            <motion.span
              className="bg-gradient-to-r from-orange-400 via-orange-200 to-white bg-clip-text text-transparent inline-block"
              animate={{
      ...
      {sentiments.map((sentiment, i) => (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm whitespace-nowrap">{sentiment.label}</span>
          </div>
        </motion.div>
      ))}
    </div>
...
function FloatingStockSymbolsWithSentiment() {
  const symbols = [
    { name: 'AAPL', change: '+2.4%', positive: true, sentiment: 'Bullish' },
    { name: 'GOOGL', change: '+1.2%', positive: true, sentiment: 'Positive' },
    { name: 'MSFT', change: '+1.8%', positive: true, sentiment: 'Bullish' },
    { name: 'TSLA', change: '+5.2%', positive: true, sentiment: 'Strong Buy' },
    { name: 'NVDA', change: '+0.5%', positive: true, sentiment: 'Positive' },
    { name: 'META', change: '-1.3%', positive: false, sentiment: 'Negative' },
    { name: 'AMZN', change: '-0.3%', positive: false, sentiment: 'Neutral' },
    { name: 'NFLX', change: '+3.1%', positive: true, sentiment: 'Bullish' },
  ];
      ...
      {symbols.map((symbol, i) => (
        ...
        >
          <div className="space-y-1">
            <div className={`flex items-center gap-3 ${symbol.positive ? 'text-green-400' : 'text-red-400'}`}>
              <motion.div
                animate={{
              ...
              >
                {symbol.positive ? <ArrowUp className="w-5 h-5" /> : <ArrowDown className="w-5 h-5" />}
              </motion.div>
              <span className="text-lg">{symbol.name}</span>
            </div>
            <div className="flex items-center justify-between gap-3 text-sm">
              <span className={symbol.positive ? 'text-green-300' : 'text-red-300'}>{symbol.change}</span>
